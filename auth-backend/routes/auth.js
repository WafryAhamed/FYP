...
router.post("/register", async (req, res) => {
  try {
    ...
    // Insert with status=0 (pending activation)
    ...
    // Generate & store activation token
    const activationToken = generateToken();
    await pool.query("UPDATE users SET activation_token = ? WHERE id = ?", [activationToken, result.insertId]);

    // In real app: send email with http://localhost:5173/activate?token=${activationToken}
    console.log(`[DEV] Activation link: http://localhost:5173/activate?token=${activationToken}`);
...
router.get("/activate", async (req, res) => {
  ...
  try {
    ...
    res.send(`
      <div style="font-family: Arial, sans-serif; text-align: center; padding: 40px; max-width: 600px; margin: 0 auto;">
        <h1 style="color: #0d6efd;">✅ Account Activated!</h1>
        <p style="font-size: 1.2rem; margin: 20px 0;">Your account is now active.</p>
